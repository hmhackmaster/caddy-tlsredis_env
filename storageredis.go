...
func (rd *RedisStorage) GetConfigValue() {
	...
	rd.Host = configureString(EnvNameRedisHost, rd.Host, DefaultRedisHost)
	rd.Port = configureString(EnvNameRedisPort, rd.Port, DefaultRedisPort)
	rd.DB = configureInt(EnvNameRedisDB, rd.DB, DefaultRedisDB)
	rd.Timeout = configureInt(EnvNameRedisTimeout, rd.Timeout, DefaultRedisTimeout)
	rd.Username = configureString(EnvNameRedisUsername, rd.Username, DefaultRedisUsername)
	rd.Password = configureString(EnvNameRedisPassword, rd.Password, DefaultRedisPassword)
	rd.TlsEnabled = configureBool(EnvNameTLSEnabled, rd.TlsEnabled, DefaultRedisTLS)
	rd.TlsInsecure = configureBool(EnvNameTLSInsecure, rd.TlsInsecure, DefaultRedisTLSInsecure)
	rd.KeyPrefix = configureString(EnvNameKeyPrefix, rd.KeyPrefix, DefaultKeyPrefix)
	rd.ValuePrefix = configureString(EnvNameValuePrefix, rd.ValuePrefix, DefaultValuePrefix)
	rd.AesKey = configureString(EnvNameAESKey, rd.AesKey, DefaultAESKey)
